...
- Voice command layer: parses user commands into text plans, clear/pause/resume commands, or drawing requests.
...
The first offline adapter design from `DrawingPathPlan` to `CanonicalPathPlan` now exists as a tested foundation. Sketch Centerline Mode is enabled for robot drawing via `POST /api/preview` followed by `POST /api/draw` for high-contrast sketch images. Future PNG, SVG, photo, and voice pipelines should emit `DrawingPathPlan` first, then connect to runtime only after pipeline tests and parity checks exist.
...
- preview via `POST /api/preview`, then draw via `POST /api/draw` using the returned `preview_id`
...
4. Add a voice parser that maps commands to existing backend actions or future plan builders.
5. Add a metrics dashboard section backed by existing diagnostics and future pipeline metrics.
6. Extend four-cable diagnostics and evaluation only after the kinematic supervisor behavior is covered by tests.
